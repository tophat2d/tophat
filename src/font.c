...
static bool f_pack_attempt(th_font *font, uint32_t char_offset, th_font_atlas_page *out_page, uint8_t *out_buffer, uint32_t out_buffer_size) {
	...
	if (stbtt_PackBegin(&pack_context, out_buffer, out_buffer_size, out_buffer_size, out_buffer_size, 1, NULL) == false) {
		th_error("Failed to begin pack!");
		return false;
	}

	stbtt_PackSetOversampling(&pack_context, PACK_OVERSAMPLING, PACK_OVERSAMPLING);
	if (stbtt_PackFontRange(&pack_context, font->data, 0, font->size, char_offset, PACK_CHARSPERPAGE, out_page->pc) == false) {
...
static bool f_pack_atlas_page(th_font *font, uint32_t char_offset, th_font_atlas_page *out_page) {
	bool packed = false;
	...
	for (int size = 1; !packed && size < PACK_MAXRES; size *= 2) {
		...
		if (f_pack_attempt(font, char_offset, out_page, buffer, size)) {
			...
			for (int i = 0; i < size*size; ++i) {
				rgba[i] = 0x00FFFFFF | ((uint32_t)buffer[i] << 24);
			}
...
static th_font_atlas_page *f_get_page(th_font *font, uint32_t page_index) {
	if (page_index >= PACK_PAGECOUNT) {
		th_error("Attempt to access font page index at or over "STRINGIFY(PACK_PAGECOUNT)". Oh dear.");
	}
	...
	if (pages[page_index] == NULL) {
		...
		if (f_pack_atlas_page(font, page_index*PACK_CHARSPERPAGE, pages[page_index]) == false) {
			th_error("I failed allocating page %d for the font atlas, make sure you don't put the font size too big.", page_index);
...
static th_rect f_get_dest_rect(th_font *font, stbtt_aligned_quad quad) {
	return (th_rect){quad.x0, quad.y0 + font->ascent*font->scale, quad.x1 - quad.x0, quad.y1 - quad.y0};
}
...
static f_iterator f_iterator_begin(th_font *font, const char *string, float x, float y, float scale) {
...
static stbtt_aligned_quad f_iterator_next_quad(f_iterator *iter, th_font_atlas_page **out_page) {
	uint32_t glyph;
	...
	iter->x += stbtt_GetCodepointKernAdvance(&iter->font->info, iter->prev_glyph, glyph)*iter->font->scale;
	stbtt_GetPackedQuad(page->pc, page->img.dm.x, page->img.dm.y, glyph-(glyph/PACK_CHARSPERPAGE)*PACK_CHARSPERPAGE, &iter->x, &iter->y, &quad, 1);

	// NOTE(skejeton): The letters will look crooked if i don't do this, especially applies to something like "|||||"
	iter->x = floor(iter->x);
...
static f_glyph f_iterator_next_glyph(f_iterator *iter) {
	...
	q.v[2] = (th_vf2){{rect.x+rect.w, rect.y+rect.h}}; // Third value in quad is bottom right..
...
static void* read_font_data(char *path) {
...
static void free_font(UmkaStackSlot *p, UmkaStackSlot *r) {
